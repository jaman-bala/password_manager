      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gradient-to-r from-gray-50/80 to-blue-50/50 border-b border-gray-200/50 backdrop-blur-sm">
            <tr>
              <th className="text-left py-6 px-8 font-bold text-gray-800 text-sm uppercase tracking-wider">Название</th>
              <th className="text-left py-6 px-8 font-bold text-gray-800 text-sm uppercase tracking-wider">URL</th>
              <th className="text-left py-6 px-8 font-bold text-gray-800 text-sm uppercase tracking-wider">Логин</th>
              <th className="text-left py-6 px-8 font-bold text-gray-800 text-sm uppercase tracking-wider">Пароль</th>
              <th className="text-left py-6 px-8 font-bold text-gray-800 text-sm uppercase tracking-wider">Описание</th>
            ...
            {entries.map((entry) => (
              <tr key={entry.id} className="border-b border-gray-100/50 hover:bg-gradient-to-r hover:from-blue-50/30 hover:to-purple-50/30 transition-all duration-300 group">
                <td className="py-6 px-8">
                  <div className="font-bold text-gray-900 group-hover:text-blue-700 transition-colors duration-300">{entry.title}</div>
                </td>
                <td className="py-6 px-8">
                  {entry.url ? (
                ...
                <td className="py-6 px-8">
                  <div className="flex items-center gap-2">
                    <span className="text-gray-900 font-medium">{entry.login}</span>
                    <button
                ...
                <td className="py-6 px-8">
                  <div className="text-gray-600 text-sm max-w-xs truncate font-medium">
                    {entry.notes || '—'}
                  </div>
                </td>
                <td className="py-6 px-8">
